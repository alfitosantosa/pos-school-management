              ...
              <div className="flex flex-col items-start flex-1 min-w-0">
                ...
                {(selectedStudent.nisn || selectedStudent.class?.name) && (
                  <span className="text-xs text-muted-foreground truncate w-full">
                    {[selectedStudent.nisn ? `NISN: ${selectedStudent.nisn}` : null, selectedStudent.class?.name]
                      .filter(Boolean)
                      .join(" · ")}
                  </span>
                )}
            ...
            <div className="p-2">
              {filteredStudents.length === 0 ? (
                <div className="py-6 text-center text-sm text-muted-foreground">
                  {searchTerm ? "Tidak ada siswa yang ditemukan" : "Tidak ada data siswa"}
                </div>
              ) : (
                filteredStudents.map((student) => (
                  <div
                    ...
                    className={cn(
                      "flex items-center justify-between w-full gap-2 px-2 py-3 rounded-md cursor-pointer hover:bg-accent",
                    ...
                    <div className="flex flex-col flex-1 min-w-0">
                      ...
                      {(student.nisn || student.class?.name) && (
                        <span className="text-xs text-muted-foreground truncate">
                          {[student.nisn ? `NISN: ${student.nisn}` : null, student.class?.name]
                            .filter(Boolean)
                            .join(" · ")}
                        </span>
                      )}
